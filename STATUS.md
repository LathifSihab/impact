...
| 22 | **⚠ The Astro waitlist route does not enforce guardian consent** | Found 8 Sep by testing the merged branch: `POST /api/waitlist` without the consent tick returns `200`. The deployed Netlify function returns `422` and refuses. The form collects a child's name and age, so this must match the function before the Astro app is deployed anywhere. Small fix, high consequence — see `TESTING.md` §0e. |
