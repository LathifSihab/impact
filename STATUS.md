...
| 12 | **Astro page parity** — 6 of 20 pages built (`index`, `events`, 4 × `events/[slug]`) | Verified by building it 8 Sep. over, samenwerken, social-impact, journal, media, contact and hosted-experiences exist only as static HTML, and no English routing is built. Switching `netlify.toml` to publish `web/dist` today would ship a site missing most of its pages. |
...
| 22 | ~~The Astro waitlist route does not enforce guardian consent~~ | ✅ **Fixed 8 Sep.** Found by testing the merged branch: `POST /api/waitlist` accepted a submission with no consent and answered `200`, and the event page had no consent checkbox at all — it was missing on both sides, not just the server. Now the tick is rendered unticked by default and re-checked server-side, matching `netlify/functions/subscribe.mjs`, and it is stored on the entry so the consent can be evidenced later. Verified: `422` without, `200` with. |
